import React from 'react';
import { Lock } from 'lucide-react';
import { GoogleAuthProvider, signInWithPopup, signInAnonymously } from 'firebase/auth'; // Added signInAnonymously
import { auth } from './firebase'; // Fixed import path from '../firebase' to './firebase' assuming same dir structure or check relative path

import { seedDatabase } from './services/contentService'; // Import seed service

const Login = ({ onOfflineLogin }) => {
  const [seedingStatus, setSeedingStatus] = React.useState(null); // 'loading', 'success', 'error'

  const handleLogin = async () => {
    if (!auth) {
      console.warn("Auth not initialized. Check firebase configuration.");
      // Fallback if auth is totally missing
      if (onOfflineLogin) onOfflineLogin();
      return;
    }

    try {
      const provider = new GoogleAuthProvider();
      await signInWithPopup(auth, provider);
    } catch (error) {
      console.error("Login failed:", error);
      if (error.code === 'auth/popup-blocked' || error.code === 'auth/operation-not-supported-in-this-environment') {
        // Fallback for restrictive environments
        await signInAnonymously(auth);
      } else {
        alert(`Authentication failed: ${error.message}`);
      }
    }
  };

  const handleSeed = async () => {
    if (!auth.currentUser) return;
    if (!window.confirm("Overwrite database with local content?")) return;

    setSeedingStatus('loading');
    try {
      const count = await seedDatabase();
      setSeedingStatus('success');
      alert(`Database seeded successfully! (${count} items)`);
      // Reload to see changes immediately if desired, or just let App fetch on next visit
      window.location.reload();
    } catch (e) {
      console.error(e);
      setSeedingStatus('error');
      alert(`Seeding failed: ${e.message}`);
    } finally {
      setSeedingStatus(null);
    }
  };

  return (
    <div className="flex flex-col items-center justify-center min-h-[50vh] p-4 bg-neutral-950 text-white font-sans animate-in fade-in zoom-in-95 duration-300">
      <div className="bg-neutral-900 border border-neutral-800 p-8 rounded-2xl text-center max-w-md w-full shadow-2xl">
        <div className="bg-neutral-800 w-16 h-16 rounded-full flex items-center justify-center mx-auto mb-6">
          <Lock className="w-8 h-8 text-rose-500" />
        </div>
        <h2 className="text-2xl font-bold text-white mb-2">Restricted Access</h2>
        <p className="text-neutral-400 mb-8">
          This area requires Director-level clearance. Please authenticate to continue.
        </p>

        {auth?.currentUser ? (
          <div className="space-y-4">
            <p className="text-green-500 font-mono">Identity Verified: {auth.currentUser.email || 'Director'}</p>
            <button
              onClick={handleSeed}
              disabled={seedingStatus === 'loading'}
              className="w-full bg-neutral-800 text-white font-bold py-2 px-6 rounded-lg hover:bg-neutral-700 transition-colors border border-neutral-700"
            >
              {seedingStatus === 'loading' ? 'Seeding...' : '⚡ Seed Database (Admin)'}
            </button>
          </div>
        ) : (
          <button
            onClick={handleLogin}
            className="w-full bg-white text-neutral-950 font-bold py-3 px-6 rounded-lg hover:bg-neutral-200 transition-colors flex items-center justify-center gap-2"
          >
            <svg className="w-5 h-5" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
              <path d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92c-.26 1.37-1.04 2.53-2.21 3.31v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.09z" fill="#4285F4" />
              <path d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z" fill="#34A853" />
              <path d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.07H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.93l2.85-2.22.81-.62z" fill="#FBBC05" />
              <path d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.07l3.66 2.84c.87-2.6 3.3-4.53 6.16-4.53z" fill="#EA4335" />
            </svg>
            Sign in with Google
          </button>
        )}
      </div>
    </div>
  );
};

export default Login;